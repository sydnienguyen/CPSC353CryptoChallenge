...
int main()
{
    ...
    // converts encrypted text's binary string to hex 
    for (int i = 0; i < initialBinaryString.length() / 4; i++)
    {
        encrypted_text_hex.append(BinaryToHex(initialBinaryString.substr(i * 4, 4)));
    }

    // decrypts encrypted text and gets the answer 
    for (int i = 0; i < encrypted_text_hex.length()/32; i++)
    {
        DecryptAES128Bit(encrypted_text_hex.substr(i*32,32), expandedKeyArray, decryptedArray);
